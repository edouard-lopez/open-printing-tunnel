...
test('should create a site', t => {
	...
	nock('http://localhost/').post('/api/sites/', site).reply(201, siteGetOne);
...
test('should get all sites with parameters', t => {
	nock('http://localhost/').get('/api/sites/').reply(200, sitesGetAll);
...
test('should get a site', t => {
	nock('http://localhost/').get('/api/sites/bordeaux/').reply(200, siteGetOne);
...
test('should update a site', t => {
	nock('http://localhost/').put(`/api/sites/${siteGetOne.id}/`).reply(200, siteGetOne);
...
test('should delete a site', t => {
	nock('http://localhost/').delete(`/api/sites/${siteGetOne.id}/`).reply(200);
