...
export default {
  ...
  getConfigurePortsScript({ dispatch }, site) {
    return scripts
      .get({ site: site.id }, { url: '/api/${resource}/${site}/ports/' })
      .then(response => {
        logging.success('Génération du script réussi.');
        return response;
      })
  ...
  deleteSite({ dispatch }, site) {
    return sites
      ...
      .catch(() => {
        logging.error(
          "Impossible de supprimer ce site pour l'instant. Retentez dans quelques instants ou contacter un administrateur"
        );
      });
  },
  deletePrinter({ dispatch }, site, printer) {
    const printers = resource('printers', {
      delete: '/api/sites/${site}/${resource}/${id}/'
    });

    return printers
      .delete({ site: site.id, id: printer.id })
      .then(() => {
  ...
  scanSite({ dispatch }, site) {
    return scan
      .get({ site: site.hostname }, { url: '/api/${resource}/${site}/' })
      .then(response => {
        ...
        if (found === 0) {
          logging.warning(
            'Scan du réseau terminé : aucune imprimante trouvée.'
          );
        } else {
          logging.success(
            'Scan du réseau terminé : ' + found + ' imprimantes trouvées.'
          );
        }
        return response;
      })
  ...
  getSiteConfig({ dispatch }, site) {
    return siteConfig
      .get({ site: site.id }, { url: '/api/${resource}/${site}/' })
      .then(response => {
        dispatch('logResponse', response);
